...
Спан продюсера заканчивается на коммите его транзакции. Спан консьюмера
начинается, когда брокер отдаёт ему сообщение. Между ними — разрыв ровно
шириной в задержку outbox, и метрика скажет, что он велик, но не скажет, из чего
он состоит.

Укажите диспетчеру коллектор, и разрыв заполнится:

```bash
OUTBOX_OTEL_ENDPOINT=otel-collector:4318
OUTBOX_OTEL_INSECURE=true          # коллектор рядом с процессом
```

Каждое опубликованное сообщение получает один спан `outbox.publish`,
родительский контекст которого взят из `traceparent` продюсера, а результат
записан обратно в заголовки сообщения — чтобы консьюмер продолжил уже от него.
Трейс читается как продюсер → `outbox.publish` → консьюмер, в одном трейсе, а
ожидание видно как пространство перед средним спаном.

| Атрибут | Смысл |
|---|---|
| `messaging.system` | `rabbitmq` или `kafka` — на это опираются типовые представления для очередей. |
| `messaging.destination.name` | Топик, в который ушло сообщение. |
| `messaging.message.id` | Id строки outbox — то же, по чему дедуплицирует консьюмер. |
| `outbox.stream`, `outbox.driver` | Какой пайплайн и какой брокер это обработали. |
| `outbox.attempts` | Сколько раз брокер уже отказал — переотправленное сообщение опознаётся. |
| `outbox.wait_seconds` | Время между коммитом и этой публикацией. Спан покрывает только публикацию, поэтому без этого атрибута ожидание видно лишь как пустое место. |

Две вещи, которые стоит знать до включения.

**Продюсер, который никогда не трейсил, всё равно получает спан**, а его
консьюмер — заголовок, от которого можно продолжить. Требовать, чтобы продюсер
трейсил первым, значило бы сделать это полезным только там, где оно нужно меньше
всего.

**Заголовок меняется.** При включённом трейсинге до брокера доезжает
`traceparent`, называющий спан диспетчера, а не продюсера. Id трейса не меняется,
поэтому ничто не покидает трейс продюсера — переезжает только родитель, и именно
это ставит диспетчер между двумя концами, а не рядом с ними. При выключенном
трейсинге заголовок проходит нетронутым, ровно как раньше.

### Во что это обходится

Измерено, а не оценено. Выключенным — бесплатно: цикл публикации проверяет один
булев флаг и не открывает спанов, **0 аллокаций и около 5 нс** на сообщение.
Включённым спан стоит примерно **1.9 мкс и 17 аллокаций** на сообщение — около
1.4% одного ядра на той пропускной способности, которую показывает `make bench`.

Настоящая цена — размер бинарника. SDK OpenTelemetry и кодировщик OTLP
добавляют **6.3 МБ к бинарнику в 15.5 МБ** и 8.5 МБ к образу — независимо от
того, настроен ли когда-нибудь коллектор. gRPC попадает в `go.mod` как косвенное
требование proto-модуля OTLP, но ни один его пакет не импортируется, поэтому в
сборку он не входит.
