...
| `outbox_stream_paused`             | gauge     | stream                  | `1`, пока диспетчер перестал забирать работу для стрима из-за недоступного брокера. Claim'ы возобновятся сами, как только один пройдёт. Это сигнал, переживающий то, о чём сообщает: пока он `1`, ничего не публикуется, и `outbox_messages_deferred_total` стоит. |
...
[`dashboards/outbox.json`](../dashboards/outbox.json) — дашборд Grafana для
метрик выше. Импортируется через **Dashboards → New → Import** или раскладывается
файлом; при импорте спрашивает источник данных и больше ничего не требует.

Тринадцать панелей в четырёх рядах: успевает ли доставка, пропускная способность
и задержка, что идёт не так, и детали за этим. Переменная `stream` сужает любую
панель до одного стрима — тот самый вид, который нужен, когда сконфигурировано
несколько брокеров, а проблема в одном из них.

Он сверяется с кодом, а не со скриншотом. Тест обходит каждый запрос в файле и
падает, если тот называет метрику, которую диспетчер не регистрирует, или
фильтрует по метке, которой у этой метрики нет, — и то и другое рисует пустую
панель, которая читается как «ничего не происходит» и неотличима от хороших
новостей.
  ...
  - name: outbox
    rules:
      ...
      - alert: OutboxBrokerUnreachable
        # Два выражения, потому что первое намеренно замолкает. Как только
        # диспетчер перестаёт забирать работу для недоступного брокера, он ничего
        # не публикует — а значит, и ничего не откладывает, и rate падает в ноль
        # ровно тогда, когда авария наиболее устойчива. Истинным остаётся
        # outbox_stream_paused; rate закрывает промежуток до остановки claim'ов и
        # стримы, слишком тихие, чтобы пауза удерживалась.
        expr: >
