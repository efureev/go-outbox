...
| `OUTBOX_DISPATCH_PAUSE_MAX`          | `30s`        | The longest the dispatcher waits before trying a stream whose broker is unreachable again. The pause starts at one poll interval and doubles up to this. `0` keeps claiming throughout an outage. |
