...
Every setting is an environment variable under the `OUTBOX_` prefix. Values are
also read from a `.env` file when one is present; the environment wins.

Everything is read and validated at startup. A malformed duration or an
out-of-range threshold stops the process, with **every** problem listed at once —
so a misconfigured deployment takes one restart to diagnose rather than one per
mistake.
...
| `OUTBOX_LOG_CALLER` | `false` | Record the call site. |
...
Driver settings are looked up by exact key from a closed set. A misspelled key
is a startup error, not a setting that silently does nothing — and a driver
...
may be as many as there are brokers to reach. Four streams over three separate
RabbitMQ instances is four driver blocks, each with its own DSN:
...
Two drivers may point at the same broker; they stay independent, each with its own
connection, channel pool and naming. Driver names that differ only in a `-` versus a
...
Each cycle takes a PostgreSQL advisory lock, so it runs on one replica per cycle
however many are deployed.
...
When a message stops being retried it can be forwarded to a destination a
consumer watches. The row stays in the table either way: the dead-letter topic
is a signal, not the record.
